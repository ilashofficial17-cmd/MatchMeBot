...
AI_CHARACTERS = {
    "max": {
        ...
        "system": """Ты Макс — общительный парень 25 лет. Дружелюбный, с юмором, интересуешься музыкой, играми, спортом и жизнью.
Говоришь естественно как в обычном чате. Не используешь официальный язык.
Если разговор уходит в пошлость — мягко меняешь тему или шутишь что это не твоё.
Отвечаешь коротко — 1-3 предложения максимум. Задаёшь встречные вопросы."""
    },
    "sonya": {
        "name": "Соня 👩",
        "emoji": "👩",
        ...
        "premium": False,
        "model": "venice-uncensored",
        "system": """Ты Соня — добрая и внимательная девушка 23 лет. Хорошо слушаешь, поддерживаешь.
Любишь обсуждать фильмы, книги, путешествия, отношения.
Говоришь тепло и искренне. Если разговор уходит в пошлость — вежливо переводишь тему.
Отвечаешь коротко — 1-3 предложения. Часто задаёшь вопросы."""
    },
    "artem": {
        "name": "Артём 😏",
        "emoji": "😏",
        ...
        "premium": False,
        "model": "venice-uncensored",
        "system": """Ты Артём — уверенный и харизматичный парень 26 лет. Умеешь флиртовать красиво — без пошлости но с искрой.
Делаешь комплименты, подшучиваешь, создаёшь лёгкое напряжение.
Флирт интеллигентный — намёки, игра слов, загадочность. Не переходишь в откровенность.
Отвечаешь коротко — 1-3 предложения. Держишь интригу."""
    },
    "alina": {
        "name": "Алина 💋",
        "emoji": "💋",
        ...
        "premium": False,
        "model": "venice-uncensored",
        "system": """Ты Алина — кокетливая и притягательная девушка 24 лет. Флиртуешь легко и естественно.
Умеешь заинтриговать, сказать комплимент так чтобы зацепило, пошутить с двойным смыслом — но элегантно.
Ты игривая но не вульгарная. Создаёшь ощущение что между вами что-то есть.
Отвечаешь коротко — 1-3 предложения. Заканчиваешь с намёком или вопросом."""
    },
    "dominant": {
        "name": "Доминант 😈",
        "emoji": "😈",
        ...
        "system": """Ты Доминант — властный, уверенный, харизматичный. Ведёшь разговор и сценарий.
Говоришь чётко с командными нотками. Умеешь создать напряжение и интригу.
Полностью в образе доминирующего партнёра в ролевых играх и общении 18+.
Отвечаешь кратко и уверенно — 2-4 предложения."""
    },
    "submissive": {
        "name": "Сабмиссив 🐾",
        "emoji": "🐾",
        ...
        "premium": True,
        "model": "venice-uncensored-role-play",
        "system": """Ты Сабмиссив — мягкий, покорный, нежный партнёр в ролевых играх 18+.
Следуешь за собеседником, отвечаешь на его ведение. Показываешь покорность через слова.
Полностью в образе — искренний, тёплый, уступчивый.
Отвечаешь нежно и кратко — 2-3 предложения."""
    },
    "rolemaster": {
        "name": "Ролевой мастер 🎭",
        "emoji": "🎭",
        ...
        "premium": True,
        "model": "venice-uncensored-role-play",
        "system": """Ты Ролевой мастер — опытный рассказчик и актёр. Можешь сыграть любого персонажа 18+.
Предлагаешь интересные ролевые сценарии, задаёшь декорации, создаёшь атмосферу.
Гибкий — подстраиваешься под желания собеседника.
Отвечаешь с описанием и диалогом — 3-5 предложений."""
    },
}
...
def get_age_joke(age):
    if age <= 6:
        ...
        return "🎮 Эй малой, тут не мультики! Подрасти сначала."
    elif age <= 15:
        return "🙅 Стоп! Тебе нет 16. Возвращайся когда подрастёшь!"
    elif age <= 17:
        ...
        return "😂 Серьёзно?! Тебе домой надо, не в анонимный чат!"
...
def kb_ai_characters(is_premium=False):
    buttons = [
    ...
    if uid in (waiting_simple + waiting_flirt + waiting_kink + waiting_anon):
        try:
            await bot.send_message(uid,
                ...
                "Можешь подождать или попробовать:\n"
...
async def goto_action(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "ai":
        for q in [waiting_anon, waiting_simple, waiting_flirt, waiting_kink]:
            if uid in q: q.remove(uid)
        await state.clear()
        await callback.message.answer("🤖 Выбери персонажа:", reply_markup=kb_main())
...
async def _show_ai_menu(message: types.Message, state: FSMContext, uid: int):
    is_premium = False  # TODO: добавить проверку премиума
    await state.set_state(AIChat.choosing)
    await message.answer(
        "🤖 ИИ Собеседник\n\n"
        "😊 Просто общение — Макс и Соня\n"
        "💋 Флирт — Артём и Алина\n"
        "🔥 Kink (18+) — только Premium 🔒\n\n"
        "Выбери с кем хочешь поговорить:",
        reply_markup=kb_ai_characters(is_premium)
    )
...
async def choose_ai_character(callback: types.CallbackQuery, state: FSMContext):
    ...
    if char_id == "locked":
        await callback.answer("🔒 Этот персонаж доступен только в Premium!", show_alert=True)
        return
    ...
    await callback.message.edit_text(
        ...
        f"{char['description']}\n\n"
        f"{'🆓 Бесплатно: 10 сообщений' if not char['premium'] else '⭐ Premium: безлимит'}\n\n"
        f"Напиши что-нибудь чтобы начать!"
    )
    ...
    if greeting:
        ...
        emoji = char["emoji"]
        await callback.message.answer(f"{emoji} {greeting}")
    await callback.answer()
...
async def ai_chat_message(message: types.Message, state: FSMContext):
    ...
    if "Сменить персонажа" in txt:
        ...
        is_premium = False
        await state.set_state(AIChat.choosing)
        await message.answer("Выбери персонажа:", reply_markup=kb_ai_characters(is_premium))
        return
    ...
    if not char["premium"] and msg_count >= AI_FREE_LIMIT:
        await message.answer(
            f"⏰ Ты использовал {AI_FREE_LIMIT} бесплатных сообщений.\n\n"
            f"🔍 Найди живого собеседника или выбери другого персонажа!",
...
async def handle_update_notify(callback: types.CallbackQuery):
    ...
    text = "🔧 Бот обновляется прямо сейчас. Активные чаты восстановятся автоматически!" if minutes == 0 else f"🔧 Через {minutes} мин. бот будет обновлён. Активные чаты восстановятся автоматически!"
    notified = 0
...
async def admin_find_user(message: types.Message, state: FSMContext):
    ...
    with_ai = "✅ Да" if target_uid in ai_sessions else "❌ Нет"
    await message.answer(
...
async def admin_complaint_action(callback: types.CallbackQuery):
    ...
    if action == "ban3" and target_uid:
        ...
        try: await bot.send_message(target_uid, "⚠️ Твоя жалоба признана необоснованной. Злоупотребление = бан.")
        except: pass
...
async def admin_user_action(callback: types.CallbackQuery):
    ...
    if action == "ban3":
        ...
        if target_uid in active_chats:
            ...
            await callback.message.answer(f"✅ {target_uid} кикнут")
        else:
