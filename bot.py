...
# monitoring.py создаётся Архитектором — безопасный импорт
try:
    import monitoring
...
async def do_find(uid, state):
    ...
    if _use_redis:
        ...
        available_by_queue = {}  # qkey -> list[int]
        ...
        if all_pids:
            ...
            pid_to_row = {r["uid"]: r for r in rows}

            for pid, pu in pid_to_row.items():
...
async def main():
    ...
    # --- Monitoring init (safe if monitoring.py not yet created) ---
    if _has_monitoring:
        monitoring.init(bot=bot, db_pool=db_pool, admin_id=ADMIN_ID)
